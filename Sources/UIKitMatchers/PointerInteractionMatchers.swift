...
@available(iOS 13.4, *)
public func haveTheDefaultPointerInteraction() -> Nimble.Predicate<UIView> {
    return Nimble.Predicate { actual -> PredicateResult in
        ...
        if let button = view as? UIButton {
            return hasDefaultPointerInteraction(button: button)
        }
...
@available(iOS 13.4, *)
private func hasDefaultPointerInteraction(button: UIButton) -> PredicateResult {
    let message = ExpectationMessage.expectedActualValueTo("have the default pointer interaction")
